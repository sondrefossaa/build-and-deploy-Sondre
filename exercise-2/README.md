...
📖 GitHub Actions is "code as configuration", which means that you configure it by simply adding a workflow configuration file to your repository.

✏️ Create a new folder at the root of the repository called `.github`

✏️ Inside `.github`, create a new folder called `workflows`.
...
📖 Before we start doing anything useful with GitHub Actions, we need to get familiar with the basics of how it works.

✏️ Inside the `workflows` folder, create a new file named `test.yml` with the following contents:
...
✏️ Save and git commit this file. Git push all your changes so far.

📖 Commiting the workflow file in your repository and pushing it to GitHub triggers the `push` event and runs your workflow.

### 2.3 Viewing your workflow results

✏️ On GitHub.com, navigate to the main page of your repository.

✏️ Under your repository name, click "Actions".

![](images/actions-tab.png)

✏️ In the left sidebar, click the workflow you want to see (we named it `GitHub Actions Nerdschool Demo`).

![](images/actions-sidebar.png)

✏️ From the list of workflow runs, click the name of the run you want to see (`'xxx' is testing out GitHub Actions 🚀`)

![](images/actions-workflows.png)

✏️ Under **Jobs**, click the Test-GitHub-Actions job.
...
📖 Next step is to make GitHub Actions do something more useful!

❗️ - If you are not able to see the workflow, try refreshing the page.

### 2.4 Workflow for building our app

📖 The most important step in a CI-pipeline is the _build_ step, where code is built to see if a new commit pushed  _breaks the build_ or not.

📖 We are going to use the following commands inside our new pipeline to build our app:
...
- 📖 `npm ci` installs all project dependencies. This command is similar to `npm install`, except it's meant to be used in automated environments.
- 📖 `npm run build` creates a `dist` (abbriviation for "distribution") directory with a production build of your app.

💡 These commands are specific to the app we are building. If you need to build another type of app (Python, .NET, Java, etc.) in your pipeline, you need to use different build tools and commands.

✏️ Delete the file `test.yml` we previously created from the `.github\workflows` folder.

✏️ Create a new file called `main.yml` in the `.github\workflows` folder with the following contents:
...
✏️ Commit all changes and push them to GitHub. The new workflow will now trigger.

✏️ Open the GitHub Actions workflow overview and click on the new `Build` workflow on the left side. Click on the workflow run on the right side to see more details.

📖 A workflow run will either succeed or fail, depending on if any of the job steps fail.
...
✏️ In a terminal window, cd into the `code` folder and run `npm run lint` to run ESLint. The command should take a few seconds, then exit without errors.

✏️ Open `code/src/main.js` and add the following line to the end of the file:

```javascript
const unusedVariable = 3;
```

✏️ Save the file, commit the change and run `npm run lint` again. You should see the following error on the screen:
...
✏️ To make our CI pipeline automatically lint code, we need to add the following line to `.github/workflows/main.yml`:
...
✏️ Git commit this change and push. View the result in the Actions workflow jobs overview on GitHub.com.

📖 The build should fail at this point, because it did not pass our linting step.

✏️ Remove the unused variable that we introduced, and run `npm run lint` again in your terminal to ensure that the code is linted successfully. Push the change to Github and see that the build is successful.

### 2.5.2 Testing

📖 There is already a script for running our unit tests in `package.json`.

✏️ Run `npm test` and see that it succeeds. If the terminal says "No tests found related to files changed since last commit" or something similar, press the `a` key to make it run all tests regardless. Press the `q` key to exit. When running this script on our CI, it won't enter this REPL loop or watch mode. It'll just run through all tests once and exit the script.

✏️ To run tests automatically in our CI pipeline, we need to add the following line to `.github/workflows/main.yml`:
...
⭐️ Bonus task: Try messing around with the tests in `code/src/clock.test.js`. Can you make them fail? Or even create new tests that are run in our CI pipeline?
...
Commit and push your change to Github, and you chould see the following output on the bottom of you r workflow page. This is our application that has been built and is ready to be deployed.

 ![Artifact example](./images/actions-artifact.png).

---

Well done so far 🎉! Next, we'll deploy the app.

### [Go to exercise 3 ➡️](../exercise-3/README.md)
